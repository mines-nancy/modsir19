...
def model_diff(parameters: Dict[str, any], series: List[str] = None, **kwargs: Dict[str, any]) -> Dict[str, any]:
    ...
    R0_start = parameters['beta'] * parameters['dm_r'] if not 'R0_start' in parameters.keys() else parameters[
        'R0_start']
    R0_confinement = parameters['beta_post'] * parameters['dm_r'] if not 'R0_confinement' in parameters.keys() else \
